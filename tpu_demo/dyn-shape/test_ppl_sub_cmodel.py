...
def dynamic_sub(
    ...
    with T.Kernel(1, 1, is_cpu=True) as (bx, by):
        ...
        T.ppl_subtract(local_z, local_x, local_y)
        T.ppl_copy(local_z, Z[0, 0])


kernel = tilelang.compile(dynamic_sub, out_idx=-1, target="tpu", mode="cmodel")

# Test with M=3
x1 = torch.randn(3, N).float()
y1 = torch.randn(3, N).float()
z1 = torch.zeros(3, N).float()
kernel(x1, y1, z1)
assert_close("dynamic sub M=3", z1, x1 - y1)

# Test with a different M value
x2 = torch.randn(7, N).float()
y2 = torch.randn(7, N).float()
z2 = torch.zeros(7, N).float()
kernel(x2, y2, z2)
assert_close("dynamic sub M=7", z2, x2 - y2)

print("\n" + "=" * 60)
print("ALL PPL.SUB TESTS PASSED")
print("=" * 60)
