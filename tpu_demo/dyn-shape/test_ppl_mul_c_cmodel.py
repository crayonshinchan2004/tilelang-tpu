...
def dynamic_mul_c(
    ...
    with T.Kernel(1, 1, is_cpu=True) as (bx, by):
        ...
        T.ppl_mul_C(local_y, local_x, T.float32(2.0))
        T.ppl_copy(local_y, Y[0, 0])


kernel = tilelang.compile(dynamic_mul_c, out_idx=-1, target="tpu", mode="cmodel")

# Test with M=3
x1 = torch.randn(3, N).float()
y1 = torch.zeros(3, N).float()
kernel(x1, y1)
assert_close("dynamic mul_C M=3", y1, x1 * 2.0)

# Test with a different M value to verify dynamic shape works
x2 = torch.randn(7, N).float()
y2 = torch.zeros(7, N).float()
kernel(x2, y2)
assert_close("dynamic mul_C M=7", y2, x2 * 2.0)

print("\n" + "=" * 60)
print("ALL PPL.MUL_C TESTS PASSED")
print("=" * 60)
