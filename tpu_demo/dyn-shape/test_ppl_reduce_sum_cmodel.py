...
kernel = tilelang.compile(dynamic_reduce_sum, out_idx=-1, target="tpu", mode="cmodel")

# Test with M=3
x1 = torch.randn(3, N).half()
y1 = torch.zeros(3, 1).half()
kernel(x1, y1)
assert_close("dynamic reduce_sum M=3", y1, torch.sum(x1, dim=1, keepdim=True))

# Test with a different M value to verify dynamic shape works
x2 = torch.randn(7, N).half()
y2 = torch.zeros(7, 1).half()
kernel(x2, y2)
assert_close("dynamic reduce_sum M=7", y2, torch.sum(x2, dim=1, keepdim=True))

print("\n" + "=" * 60)
print("ALL PPL.REDUCE_SUM TESTS PASSED")
print("=" * 60)
