...
def dynamic_add(
    ...
    with T.Kernel(1, 1, is_cpu=True) as (bx, by):
        ...
        T.ppl_add(local_z, local_x, local_y)
        T.ppl_copy(local_z, Z[0, 0])


kernel = tilelang.compile(dynamic_add, out_idx=-1, target="tpu", mode="cmodel")
...
kernel(x1, y1, z1)
assert_close("dynamic add M=3", z1, x1 + y1)
...
kernel(x2, y2, z2)
assert_close("dynamic add M=7", z2, x2 + y2)

print("\n" + "=" * 60)
print("ALL PPL.ADD TESTS PASSED")
print("=" * 60)
